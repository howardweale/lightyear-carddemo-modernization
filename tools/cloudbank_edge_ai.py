...
def parser() -> argparse.ArgumentParser:
    root = argparse.ArgumentParser(
        description="Build, verify, materialize, or run the CloudBank edge and AI boundary"
    )
    ...
    execute.add_argument("--ms63-receipt", type=Path, required=True)
    execute.add_argument("--ms57-receipt", type=Path, required=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command in {"build", "verify"}:
        ...
        workspace = materialize_target(
            args.project_root.resolve(), args.source_root.resolve(), args.output.resolve()
        )
        result = {"status": "passed", "workspace": str(workspace)}
    elif args.command == "run":
        ms63 = json.loads(args.ms63_receipt.read_text(encoding="utf-8"))
        ms57 = json.loads(args.ms57_receipt.read_text(encoding="utf-8"))
        try:
            output = execute_edge_ai(
                args.project_root.resolve(), args.source_root.resolve(), ms63, ms57,
                args.output_root.resolve(), key, args.signer, args.run_id,
                progress=lambda message: print(f"[MS64] {message}", flush=True),
            )
