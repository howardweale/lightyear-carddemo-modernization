...
5. Inspect `pilot/reference-output/pilot.dossier.json` and verify its `content_sha256` and `analysis_sha256`.
6. Hash each path in `evidence_artifacts` and compare it with the dossier's raw `sha256`.
7. Verify every readiness check is true while `mainframe_equivalent` and `production_ready` remain
   false.
8. Confirm model qualification is false unless eight current, independently sealed evaluations—two
   for each trusted workload—and an approved successful portfolio receipt are supplied.
9. Inspect `mainframe.preflight.json`; gates 6 and 8 must be blocked and gate 7 only
   `mechanism_ready`.

## Evidence interpretation

`pilot_ready: true` means a governed evaluator can install the package, inventory approved source,
inspect that intake's bounded typed estate, verify existing development evidence, and identify the
exact live prerequisites. It does not mean the original applications were compiled or executed.
