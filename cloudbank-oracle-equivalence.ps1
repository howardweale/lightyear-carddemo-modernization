...
$Tool = Join-Path $ProjectDir "tools/cloudbank_oracle_equivalence.py"
...
switch ($Action) {
    ...
    "run" {
        if ($Remaining.Count -ne 5) {
            throw "run requires SOURCE_ROOT MS57_RECEIPT MS60_RECEIPT OUTPUT_ROOT SIGNER"
        }
        Invoke-FactoryDarkPython $Tool run --project-root $ProjectDir `
            --source-root $Remaining[0] --ms57-receipt $Remaining[1] `
            --ms60-receipt $Remaining[2] --output-root $Remaining[3] --signer $Remaining[4]
    }
    "verify-receipt" {
