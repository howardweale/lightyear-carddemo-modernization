...
$Tool = Join-Path $ProjectDir "tools/cloudbank_checks_messaging.py"
...
switch ($Action) {
    ...
    "run" {
        if ($Remaining.Count -ne 4) { throw "run requires SOURCE_ROOT MS62_RECEIPT OUTPUT_ROOT SIGNER" }
        Invoke-FactoryDarkPython $Tool run --project-root $ProjectDir --source-root $Remaining[0] --ms62-receipt $Remaining[1] --output-root $Remaining[2] --signer $Remaining[3]
    }
    "verify-receipt" {
