...
def parser() -> argparse.ArgumentParser:
    root = argparse.ArgumentParser(
        description="Build, verify, materialize, or run CloudBank production OAuth"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command in {"build", "verify"}:
        ...
        ms61 = json.loads(args.ms61_receipt.read_text(encoding="utf-8"))

        def progress(message: str) -> None:
            print(f"[MS62] {message}", flush=True)
