...
public class LightyearJourneyTest extends AbstractTestCase {
    ...
    @Test public void customerToPayment() throws Exception {
        ...
        try {
            ...
            customer.setValue("LY-EQUIV-20260926-TXN"); customer.setName("Lightyear Equivalence Customer");
            ...
            product.setValue("LY-EQUIV-20260926-TXN"); product.setName("Lightyear Equivalence Product");
            ...
            price.setPriceList(new BigDecimal("10")); price.setPriceStd(new BigDecimal("10"));
            price.setPriceLimit(new BigDecimal("10")); price.saveEx(); record("productPrice", price); commit();
            MOrderLine orderLine = new MOrderLine(order);
            orderLine.setLine(10); orderLine.setProduct(product); orderLine.setQty(new BigDecimal("2"));
            orderLine.setPrice(new BigDecimal("10")); orderLine.setDatePromised(businessDate);
            assertTrue(orderLine.setTax()); orderLine.saveEx(); complete("order", order); commit();
            ...
            assertEquals(0, new BigDecimal("20").compareTo(order.getTotalLines()));
            ...
            assertEquals(0, new BigDecimal("2").compareTo(orderLine.getQtyDelivered()));

            MInvoice invoice = new MInvoice(shipment, businessDate);
            invoice.saveEx(); MInvoiceLine invoiceLine = new MInvoiceLine(invoice);
            invoiceLine.setShipLine(shipmentLine); invoiceLine.setQty(new BigDecimal("2"));
            ...
            fact("inventory.onHand", balance); assertEquals(0, new BigDecimal("8").compareTo(balance));
            getTrx().commit(true);
