...
- `dossiers/carddemo-intcalc-v0.14-demo.json`: current machine-readable release evidence dossier;
- `dossiers/carddemo-intcalc-v0.14-demo.md`: current human-readable dossier;
...
The v0.14 canonical demo contains 16 events: graph and source-evidence publication, verified-memory
snapshot publication, work-order registration, normalized hardened-execution evidence, three distinct
...
PYTHONPATH=src python3 -m lightyear_audit inspect \
  --decision decision:release:carddemo-intcalc:v0.14-demo:promotion
```
