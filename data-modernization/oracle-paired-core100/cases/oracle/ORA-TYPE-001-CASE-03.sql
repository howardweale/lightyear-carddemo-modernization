-- Catalog case ORA-TYPE-001-CASE-03; lane 26ai; canonical semantics; session-version
...
BEGIN
  ...
  EXECUTE IMMEDIATE 'ALTER SESSION SET NLS_NUMERIC_CHARACTERS = ''.,''';
  SELECT JSON_OBJECT('case_id' VALUE 'ORA-TYPE-001-CASE-03', 'observations' VALUE
    JSON_OBJECT('arithmetic' VALUE v_sum,
      'nls_comma' VALUE v_comma,
      'nls_dot' VALUE v_dot NULL ON NULL) RETURNING VARCHAR2(4000)) INTO v_json FROM dual;
