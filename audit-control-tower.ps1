...
if ($action -eq "build") {
    ...
    $generatedJson = Join-Path $verificationDir "carddemo-intcalc-v0.17-demo.json"
    $generatedMarkdown = Join-Path $verificationDir "carddemo-intcalc-v0.17-demo.md"
    ...
    & py -3.11 -m lightyear_audit compare `
        ...
        --actual $generated
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    $canonicalJson = (Get-FileHash (Join-Path $projectDir "audit/dossiers/carddemo-intcalc-v0.17-demo.json") -Algorithm SHA256).Hash
    $actualJson = (Get-FileHash $generatedJson -Algorithm SHA256).Hash
    $canonicalMarkdown = (Get-FileHash (Join-Path $projectDir "audit/dossiers/carddemo-intcalc-v0.17-demo.md") -Algorithm SHA256).Hash
