...
(() => {
  ...
  function render(value) {
    if (value.campaign_id !== window.LightyearContext.state.campaignId) throw new Error('Unexpected campaign response');
    host.replaceChildren(node('p', `Campaign preparation · ${value.current_run_status === 'unavailable' ? 'Not run' : value.current_run_status}`, 'campaign-eyebrow'), node('h2', value.name), node('p', value.scope));
    ...
    readiness.append(node('p', 'This readback does not check Oracle runtime, worker capacity, application health or database connectivity. The run records its own database identity checks. A READY or RUNNING resource is not permission to run.'));
    ...
    const next = node('section'); next.append(node('h3', 'Execution readiness'));
