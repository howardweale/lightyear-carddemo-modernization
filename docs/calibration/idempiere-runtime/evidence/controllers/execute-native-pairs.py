...
before={'oracle':json.loads((out/'entry/oracle/state.json').read_text()),'postgresql':json.loads((out/'entry-verified/postgresql/state.json').read_text())}
...
assert len(pairs)==20
plan=seal({'scope':'current imported seed states; common pending paired migrations only','pairs':pairs,'ordering':'upstream sorted full paths','stop_on_execution_failure':True,'historical_entry_states_verified':False,'entry_state_sha256':{k:v['content_sha256'] for k,v in before.items()}})
with (out/'execution-plan.json').open('x') as f:json.dump(plan,f,indent=2)
