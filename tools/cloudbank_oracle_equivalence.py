...
def parser() -> argparse.ArgumentParser:
    root = argparse.ArgumentParser(
        description="Build, verify, materialize, or run CloudBank Oracle equivalence"
    )
    ...
    execute.add_argument("--ms60-receipt", type=Path, required=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command in {"build", "verify"}:
        ...
        oracle, postgres = materialize_workspaces(
            args.project_root.resolve(), args.source_root.resolve(), args.output.resolve()
        )
        result = {
            "status": "passed",
            "oracle": str(oracle),
            "postgresql": str(postgres),
        }
    elif args.command == "run":
        ms57 = json.loads(args.ms57_receipt.read_text(encoding="utf-8"))
        ms60 = json.loads(args.ms60_receipt.read_text(encoding="utf-8"))

        def progress(message: str) -> None:
            print(f"[MS61] {message}", flush=True)
