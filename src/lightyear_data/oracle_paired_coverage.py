...
def project(root, entries):
    ...
    return {'status': 'verified-paired-catalog-overlay', 'contract': 'bounded-paired-probes-v1',
            ...
            'coverage_statement': f"Native Oracle 26ai–AlloyDB PostgreSQL: {paired_count} unique equivalent case pairs across {paired_behaviors} bounded behaviours. Under the paired probe contract, {source_count} unique Oracle 26ai cases were observed and {native_behaviors} behaviours verified. Separately, {baseline['bounded_model_verified_behavior_count']}/{baseline['catalogued_behavior_count']} catalog behaviours have bounded-model coverage. Repeated case IDs across campaigns are deduplicated.",
            'gate_note': 'The separate MS51 native_executed_case_count belongs to its committed catalog readiness snapshot, not a project-wide execution total. That gate requires 2,000 cases on each of Oracle 19c and 26ai under its wallet and per-case harness receipt contract; it does not aggregate these paired-campaign receipts. AlloyDB executions never count as Oracle executions. This campaign adds no platform qualification claim and does not revoke the separate CloudBank AlloyDB nonproduction qualification.',
