...
(() => {
  ...
  function render(result) {
    ...
    const state = { completed: 'Completed within declared scope', paused: 'Paused at a durable checkpoint', halted: `Stopped · ${result.halt_reason}`, running: result.activity === 'recent-engine-activity' ? 'Recent engine activity' : 'Engine activity is unobserved; resume from the checkpoint' }[result.status] || result.status;
    $('execution-status').textContent = `${source} · ${state} · ${new Date(result.last_event_at).toLocaleString()}`;
    const s = result.summary;
    for (const [title, count] of [['Verified service evidence', `${s.resolved} / ${s.services}`], ['Unresolved services', s.unresolved], ['Actions executed', s.actions_executed], ['Iterations', s.iterations]]) {
