...
def replay_idempiere(source,retained,manifest,output, *, context=None):
    ...
    results=[];parser_results=[];transitions=Counter();changed=[];lost=[]
    for item in inputs:
    ...
    measurement=seal({'schema_version':'1.0','artifact_type':'lightyear-calibration-measurement',
        ...
        'context_baseline_sha256':digest(acquisition),'context_mode':'acquisition-template-not-runtime-facts',
        ...
        'schema_session_inventory':{'cases':len(acquisition['cases']),
           ...
           'complete_catalogs':0,'verified_sessions':0},
    ...
    if context is not None:
        (output/'context-used.json').write_text(json.dumps(context,sort_keys=True,indent=2)+'\n',encoding='utf-8')
...
def write_summary(m,output):
    ...
    lines+=['',f"**Actual lift: {a['parsed-and-compared']-b['parsed-and-compared']:,} additional decided units.** "
        ...
        f"Applied context hash: {m['applied_context_sha256'] or 'none'}. Context facts are caller-supplied contracts, not authenticated runtime state.",'',
        ...
        'Catalog completeness, triggers, constraints, row policies, rewrite rules and session settings remain explicitly unknown. '
...
<h1>Does calibration raise decidability?</h1><p class="metric">{b['parsed-and-compared']:,} → {a['parsed-and-compared']:,} decided SQL units</p>
...
<h2>What this proves</h2><p>The loop produced {a['parsed-and-compared']-b['parsed-and-compared']:,} additional decisions and reduced unsupported syntax by {b['unparsed']-a['unparsed']:,} units. No previous decision or divergence was lost. It also shows why parsing alone is insufficient.</p>
<h2>What still blocks useful coverage</h2><p>Schema, session, trigger and expression semantics still dominate. The new acquisition baseline inventories those requirements without inventing runtime facts. {m['case_counts']['after']['indeterminate']:,} complete pairs remain indeterminate.</p>
