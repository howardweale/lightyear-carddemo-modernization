...
def parser() -> argparse.ArgumentParser:
    root = argparse.ArgumentParser(
        description="Build, verify, materialize, or run the CloudBank PostgreSQL transaction core"
    )
    ...
    execute.add_argument("--ms58-receipt", type=Path, required=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command in {"build", "verify"}:
        ...
        ms58 = json.loads(args.ms58_receipt.read_text(encoding="utf-8"))
        try:
            ...
            result = {
                "status": "failed",
                "error": str(exception),
                "diagnostics": str(args.output_root / "cloudbank-transaction-core.failure.json"),
            }
    else:
